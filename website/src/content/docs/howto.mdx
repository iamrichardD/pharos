...
## Security Tiers: YOLO vs. Lockdown

Pharos supports multiple security models to match your deployment environment.

- **"The YOLO Lab" (Open / Read-Only)**: Perfect for single-user home labs. Anyone on the network can query the database without authentication. Writing still requires an authorized SSH key for basic safety.
    - **Config**: `export PHAROS_SECURITY_MODE="open"`
- **"Locking down the Lab" (Protected / Scoped)**: Recommended for shared labs or Enterprise environments. Read access is restricted via tokens or IP whitelists. In "Scoped" mode, users only see records belonging to their own team.
    - **Config**: `export PHAROS_SECURITY_MODE="scoped"`
    - **Dependency**: Requires an LDAP or OIDC backend to map identities to teams.
...
<TieredTabs>
  <div slot="homelab">
    ...
    ### 🩺 Heartbeat Monitoring with `pharos-pulse`
