...
Pharos doesn't just store static data; it automatically discovers and maintains your fleet's inventory using `pharos-pulse`.

1.  **Autonomous Heartbeats**: Every node running the `pharos-pulse` agent sends periodic identity and hardware inventory (CPU cores, Brand, OS Version) to the server.
2.  **Inventory Dashboard**: Use the [Pharos Console](./console) to visualize your entire hardware and software fleet at a glance.
3.  **Presence Tracking**: Pharos automatically marks nodes as `ONLINE`, `HEARTBEAT`, or `OFFLINE` based on the agent's lifecycle signals.
