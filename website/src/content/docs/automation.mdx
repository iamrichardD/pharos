...
1.  **Baseline (ONLINE)**: On startup, `pharos-pulse` collects a full hardware and OS metadata package (CPU Brand, Core count, Total RAM, Kernel version, Serial Number, and real network interface addresses via multi-valued `ip_addr` and `mac_addr` fields) and registers it with the server.
