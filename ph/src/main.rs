...
pub fn run_client<T: PhIo>(io: &mut T, addr: &str, command_line: &str) -> Result<Vec<String>, String> {
    io.connect(addr)?;
...
mod tests {
    ...
    fn test_should_return_formatted_output_when_query_successful() {
        ...
        let result = run_client(&mut mock, "127.0.0.1:1050", "name=john").unwrap();
        
        assert_eq!(mock.written_lines[0], "id ph");
        assert_eq!(mock.written_lines[1], "query name=john");
        assert_eq!(mock.written_lines[2], "quit");
        
        assert_eq!(result.len(), 3);
        assert_eq!(result[0], "There were 1 matches to your request.");
        assert_eq!(result[1], "          alias: jdoe");
        assert_eq!(result[2], "           name: John Doe");
    }
    ...
    fn test_should_return_error_when_connection_fails() {
        ...
        let result = run_client(&mut mock, "127.0.0.1:1050", "name=john");
    ...
    fn test_should_return_empty_when_no_matches() {
        ...
        let result = run_client(&mut mock, "127.0.0.1:1050", "name=unknown").unwrap();
