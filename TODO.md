...
- [ ] **Debt #24 (Issue #171):** Action: The `SYNC ` forwarded-marker introduced by Debt #23/Issue #170 is unauthenticated — any client with write access can spoof it to suppress both replication and their own webhook notification, bypassing the audit trail. Needs real peer authentication (mTLS or fingerprint-allowlist) to close properly.
- [ ] **Debt #25 (Issue #172):** Action: Consider redesigning Issue #59's multi-master mesh replication as a primary/secondary (leader-follower) topology instead, to structurally eliminate the whole class of ping-pong/spoofing problems in Debt #23/#24 — a bigger design question, not a quick patch.
- [ ] **Debt #26 (Issue #173):** Action: `pharos-scan` has no non-interactive/scripted output mode — `network-scan.mdx` documented a `--json`/`--filter` mode that never existed in the CLI (removed from docs; implement for real or leave unimplemented).
- [ ] **Debt #27 (Issue #174):** Action: `install.sh` mis-serves two real platforms — no `linux-aarch64` release asset exists despite being requested for detected ARM64 Linux hosts, and macOS Intel hosts are silently served the `macos-aarch64` binary regardless of detected arch (docs corrected to state actual current support; script/CI gap still open).
- [ ] **Debt #28 (Issue #175):** Action: RFC 2378 `set` session options (`limit`, `addonly`, etc.) are completely unimplemented — no server-side cap on how many records a single `change`/`delete` can affect.
- [ ] **Debt #29 (Issue #176):** Action: Response status codes conflict with RFC 2378's defined meanings in several places (403/401 used inconsistently for "not authorized"/"not logged in"; 598/599 each reused for two different conditions; 500 used generically where RFC's 517 fits the read-only case).
- [ ] **Debt #30 (Issue #177):** Action: Implement RFC 2378's `fields` (schema discovery) command — parsed since day one, never dispatched, the main interop gap for any non-Pharos Ph client.
- [ ] **Debt #31 (Issue #178):** Action: Document Pharos's known RFC 2378 compliance deviations (no field-level ACL/keyword system, RFC's native login mechanisms replaced by SSH-key auth, several parsed-but-undispatched commands) so they read as intentional, not as bugs.
- [ ] **Debt #32 (Issue #179):** Action: Full RFC 2378 wildcard support (`+`, `?`, `[abc]`, non-suffix `*`) — Issue #155/Debt #10 fixed the failure mode (loud error vs. silent wrong match) for unsupported wildcards, but only a single trailing `*` is actually supported.
