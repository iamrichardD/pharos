...
- [ ] **Debt #34 (Issue #181):** Action: `pharos-pulse` never provisions a signing key for itself — `install_pulse()` generates certs and configures the systemd unit but never creates/enrolls an identity for the pulse agent to authenticate as. Confirmed live: on a real node against a Protected-tier hub, pulse retried baseline registration every ~60-120s for 20+ minutes straight, every attempt failing with "No private key found for signing" (it runs as `User=pharos`, a system user with no home directory, so every path `sign_message_async` checks is empty). Presence registration silently never succeeds against any Protected/Scoped-tier hub. Needs design: what role/identity pulse's own key should have (Protected tier's `SecurityTierMiddleware` only checks `context.authenticated`, not roles, so full admin may not be required).
- [ ] **Debt #35 (Issue #182):** Action: `mdb`/`ph` don't support `--version` — neither CLI struct sets clap's `version` attribute. Add `#[command(version)]` to both (pulls `CARGO_PKG_VERSION` automatically, same mechanism already used for pharos-pulse's startup banner).
- [ ] **Debt #36 (Issue #183):** Action: node install's "Next Steps" never explains that `ph`/`mdb` need `PHAROS_SERVER`/`PHAROS_HOST` exported in the operator's own shell to reach the hub — that env var only ever lives inside `pharos-pulse`'s systemd unit. Confirmed live: an operator hit `mdb *` defaulting to `127.0.0.1:2378`, then hit `VAR=value` without `export` looking like it worked (`echo` shows it) while remaining invisible to `mdb`.
- [ ] **Debt #37 (Issue #184):** Action: `check_cert_ip_drift()` false-positives on certs with zero IP SANs at all (e.g. a real Let's Encrypt cert), permanently suggesting a "fix" (regenerate the cert) that would destroy a real certificate on every future `install.sh` run. Plan already written: `artifacts/ip-drift-false-positive-plan.md` — `setup_pki()` always includes at least `127.0.0.1` as an IP SAN, so zero IP SANs at all is a reliable signal the cert isn't self-signed-by-Pharos and the drift check doesn't apply.
- [ ] **Debt #38 (Issue #185):** Action: `mdb`/`ph` should interactively offer to generate (and optionally enroll) a signing key on a missing-key error instead of only printing static instructions. Plan already written: `artifacts/interactive-key-setup-plan.md` — local generation gated on `std::io::IsTerminal` (safe for scripts/CI), remote enrollment reusing `--fetch-ca-ssh`'s proven opt-in/non-interactive-safe pattern. Must live only in `mdb`/`ph`'s own `main()`, never in `pharos-client`'s shared library, since `pharos-pulse` (no controlling terminal) depends on the same code path. May share design with Debt #34/Issue #181's own key-provisioning needs.
